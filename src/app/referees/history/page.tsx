...
export default async function RefereeHistoryPage() {
  const appointments = await prisma.refereeAppointment.findMany({ where: { status: "PUBLISHED", match: { status: { not: "CANCELLED" }, kickoff: { lte: new Date() }, isTestData: false, competition: { isTestData: false } } }, include: { match: { include: { competition: true, homeTeam: true, awayTeam: true } }, positions: { include: { referee: true }, orderBy: [{ sortOrder: "asc" }, { slot: "asc" }] } }, orderBy: { match: { kickoff: "desc" } } });
