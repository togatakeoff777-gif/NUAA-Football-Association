...
import { createCombinedBackup, sqlitePathFromUrl } from "../src/lib/combined-backup";
...
async function main() {
  ...
  const capacity = await calculateBackupCapacity({ databasePath, uploadRoot, retentionCount: 1 });
  ...
  const manifest = await createCombinedBackup({ databaseUrl, uploadRoot, outputDirectory: output });
