...
export default async function RefereeDirectoryPage() {
  ...
  return <><SiteHeader /><main className="functional-page" id="main-content"><section className="functional-hero"><div className="detail-shell"><p>REGISTERED REFEREES</p><h1>注册裁判员公开名录</h1><p>仅公开赛事来源可核验的姓名、公开编号与登记赛制，不展示联系方式或其他个人敏感信息。</p></div></section><RefereeSubnav /><section className="functional-section"><div className="detail-shell">{referees.length ? <div className="referee-directory-grid">{referees.map((referee) => <article key={referee.id}><span>{referee.publicCode}</span><h2>{referee.name}</h2><p>{[referee.elevenASide ? "十一人制" : null, referee.futsal ? "五人制" : null].filter(Boolean).join(" / ")}</p></article>)}</div> : <div className="functional-empty"><strong>当前暂无经协会确认可公开的裁判员名录</strong><p>完成信息授权与核验后更新。</p></div>}</div></section></main><SiteFooter /></>;
}
