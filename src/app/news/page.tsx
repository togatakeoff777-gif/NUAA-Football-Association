...
import { demoAnnouncements, newsFeed } from "@/data/content";
    ...
    <ListPageLayout eyebrow="NEWS & NOTICES" title="新闻与公告" description="用高密度列表连接比赛战报、协会动态和正式公告，并为未来筛选与分页预留结构。" listTitle="最新发布" listDescription="2026男子足球院际杯5篇正式报道已归档；演示内容继续保留并明确标注。" statusLabel="正式报道 + 演示公告" filters={filters}>
      <div className="news-list-layout">
        <div className="news-list-stream">
          ...
          {news.map((item) => <article className="news-list-row" key={item.id}><div><Image src={item.image} alt={item.imageAlt} fill sizes="150px" /></div><time>{item.dateLabel}</time><section><span>{item.category} · {item.badge}</span><h2>{item.title}</h2><p>{item.summary}</p></section><Link href={item.href} aria-label={`查看：${item.title}`}>→</Link></article>)}
        </div>
