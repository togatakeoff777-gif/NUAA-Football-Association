...
export default async function RefereeAvailabilityPage() {
  ...
  return <><SiteHeader /><main className="functional-page" id="main-content"><section className="functional-hero"><div className="detail-shell"><p>AVAILABILITY</p><h1>我的可执裁时间</h1><p>在月历中选择日期，记录整天、不可执裁或一个以上的具体可用时段。</p></div></section><RefereeWorkspaceNav /><section className="functional-section"><div className="detail-shell"><RefereeAvailabilityCalendar records={records.map((item) => ({ id: item.id, startAt: item.startAt.toISOString(), endAt: item.endAt.toISOString(), kind: item.kind, competitionFormat: item.competitionFormat, note: item.note ?? "" }))} /></div></section></main><SiteFooter /></>;
}
