...
export function AvailabilityManager({ records, referees }: {
  ...
  return <>
    ...
    <div className="admin-table-scroll"><table className="admin-data-table">
      ...
      <tbody>{records.map((item) => <tr key={item.id}><td><strong>{item.referee}</strong></td><td><span className="admin-status-badge" data-status={item.kind}>{item.kind === "AVAILABLE" ? "可执裁" : "不可执裁"}</span></td><td>{item.competitionFormat === null ? "均可" : item.competitionFormat === "ELEVEN_A_SIDE" ? "十一人制" : "五人制"}</td><td>{item.startAt}</td><td>{item.endAt}</td><td>{item.note || "—"}</td><td><div className="admin-table-actions"><button onClick={() => run(() => api("/api/referees/admin/availability", "DELETE", { id: item.id, refereeId: item.refereeId }), "记录已删除。")} type="button">删除</button></div></td></tr>)}</tbody>
    </table></div>
