...
async function main() {
  const databasePath = path.resolve("prisma/test-referee-fix2.db");
  await removeTestDatabase(databasePath);
  const runner = path.resolve("node_modules/tsx/dist/cli.mjs");
  const worker = path.resolve("scripts/test-referee-fix2-worker.ts");
  const exitCode = await new Promise<number>((resolve, reject) => {
    const child = spawn(process.execPath, [runner, worker], {
      env: { ...process.env, REFEREE_FIX2_TEST_DATABASE_PATH: databasePath },
      stdio: "inherit",
    });
...
main().catch((error) => {
  console.error(error instanceof Error ? error.message : "Referee Fix #2 test failed.");
  process.exit(1);
});
