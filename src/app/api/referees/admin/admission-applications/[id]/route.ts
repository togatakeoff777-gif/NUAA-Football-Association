...
export async function PATCH(
  ...
  try {
    ...
    try {
      action = readEnum(body.action, ["APPROVE", "REJECT"] as const, "审核结果");
      reviewNote = readShortText(body.reviewNote, "审核意见", 500);
    } catch (error) {
      throw new UnifiedAdminInputError(
        error instanceof Error ? error.message : "审核内容格式不正确。",
      );
    }
    const { id } = await context.params;
    let application;
    if (action === "REJECT") {
      ...
      let approveInput:
        ...
        | { mode: "LINK_EXISTING"; existingRefereeId: string; initialPassword: string };
      try {
        const mode = readEnum(
          body.mode,
