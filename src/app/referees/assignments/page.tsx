...
async function getItems(): Promise<PublicAppointment[]> {
  const appointments = await prisma.refereeAppointment.findMany({ where: { status: "PUBLISHED", match: { kickoff: { gt: new Date() }, isTestData: false, competition: { isTestData: false } } }, include: { match: { include: { competition: true, homeTeam: true, awayTeam: true } }, positions: { include: { referee: true }, orderBy: { sortOrder: "asc" } } }, orderBy: { publishedAt: "desc" } });
