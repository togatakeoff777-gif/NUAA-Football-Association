...
export function DetailPageLayout({ eyebrow, title, description, meta, attachments = [], related = [], children }: DetailPageLayoutProps) {
  const aside = (
  ...
  return (
    <>
      ...
      <main className="template-page template-detail-page" id="main-content">
        <PageIntro eyebrow={eyebrow} title={title} description={description} statusLabel="演示详情 · 非正式发布" variant="detail" aside={aside} />
        <article className="template-section detail-article">
          <div className="page-shell detail-article-grid">
            ...
            <aside className="detail-article-aside">
              <section><p>ATTACHMENTS</p><h2>附件</h2>{attachments.length ? <ul>{attachments.map((item) => <li key={item}>{item}</li>)}</ul> : <span>当前没有可下载附件。</span>}</section>
