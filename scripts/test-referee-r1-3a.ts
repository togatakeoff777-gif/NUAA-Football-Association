...
async function main() {
  const databasePath = path.resolve("prisma/test-referee-r1-3a.db");
  await removeTestDatabase(databasePath);
  const runner = path.resolve("node_modules/tsx/dist/cli.mjs");
  const worker = path.resolve("scripts/test-referee-r1-3a-worker.ts");
  const exitCode = await new Promise<number>((resolve, reject) => {
    const child = spawn(process.execPath, [runner, worker], {
      env: { ...process.env, REFEREE_R1_3A_TEST_DATABASE_PATH: databasePath },
      stdio: "inherit",
    });
...
main().catch((error) => {
  console.error(error instanceof Error ? error.message : "Referee R1-3A test failed.");
  process.exit(1);
});
