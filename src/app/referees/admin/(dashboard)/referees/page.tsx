...
export default async function AdminRefereesPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const [colleges, referees] = await Promise.all([
    ...
    prisma.referee.findMany({
      ...
      select: { id: true, name: true, publicCode: true, studentId: true, refereeLevel: true, status: true, college: { select: { name: true } }, capabilities: { where: { status: { not: "NOT_ASSIGNED" } }, select: { format: true } } },
  ...
  return <>
    ...
    <AdminPanel title={`裁判员名录 · ${referees.length}`} description="列表不展示手机、QQ 和内部备注。">
      {referees.length ? <div className="admin-table-scroll"><table className="admin-data-table"><thead><tr><th>姓名</th><th>裁判员编号</th><th>学院</th><th>裁判资质</th><th>十一人制</th><th>五人制</th><th>状态</th><th>操作</th></tr></thead><tbody>{referees.map((referee) => {
