...
async function main() {
  const databasePath = path.resolve("prisma/test-referee-flow.db");
  await removeTestDatabase(databasePath);
  const runner = path.resolve("node_modules/tsx/dist/cli.mjs");
  const worker = path.resolve("scripts/test-referee-flow-worker.ts");
  const exitCode = await new Promise<number>((resolve, reject) => {
    const child = spawn(process.execPath, [runner, worker], {
      env: { ...process.env, REFEREE_TEST_DATABASE_PATH: databasePath },
      stdio: "inherit",
    });
