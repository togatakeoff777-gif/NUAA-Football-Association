...
async function main() {
  const databasePath = path.resolve("prisma/test-referee-fix3.db");
  await removeTestDatabase(databasePath);
  const runner = path.resolve("node_modules/tsx/dist/cli.mjs");
  const worker = path.resolve("scripts/test-referee-fix3-worker.ts");
  const exitCode = await new Promise<number>((resolve, reject) => {
    const child = spawn(process.execPath, [runner, worker], {
      env: { ...process.env, REFEREE_FIX3_TEST_DATABASE_PATH: databasePath },
      stdio: "inherit",
    });
...
main().catch((error) => {
  console.error(error instanceof Error ? error.message : "Referee Fix #3 test failed.");
  process.exit(1);
});
