...
export function AvailabilityManager({
  ...
  async function create(event: React.FormEvent<HTMLFormElement>) {
    ...
    const ok = await run(() => api("/api/referees/admin/availability", "POST", { refereeId: form.get("refereeId"), startAt: form.get("startAt"), endAt: form.get("endAt"), kind: form.get("kind"), competitionType: form.get("competitionType"), note: form.get("note") }), "可执裁时间已代录。");
    if (ok) setOpen(false);
  }
  return <>
    <p aria-live="polite" className="admin-form-message">{message}</p>
    <div className="admin-table-scroll"><table className="admin-data-table"><thead><tr><th>裁判员</th><th>类型</th><th>制式</th><th>开始</th><th>结束</th><th>说明</th><th>操作</th></tr></thead><tbody>{records.map((item) => <tr key={item.id}><td><strong>{item.referee}</strong></td><td><span className="admin-status-badge" data-status={item.kind}>{item.kind === "AVAILABLE" ? "可执裁" : "不可执裁"}</span></td><td>{item.competitionFormat === null ? "均可" : item.competitionFormat === "ELEVEN_A_SIDE" ? "十一人制" : "五人制"}</td><td>{item.startAt}</td><td>{item.endAt}</td><td>{item.note || "—"}</td><td><div className="admin-table-actions"><button onClick={() => run(() => api("/api/referees/admin/availability", "DELETE", { id: item.id, refereeId: item.refereeId }), "记录已删除。")} type="button">删除</button></div></td></tr>)}</tbody></table></div>
    <button className="admin-floating-create" onClick={() => setOpen(true)} type="button">+ 代录时间</button>
    {open ? <div aria-modal="true" className="admin-modal-backdrop" role="dialog"><div className="admin-modal"><header><div><span>AVAILABILITY</span><h2>管理员代录可执裁时间</h2></div><button aria-label="关闭" onClick={() => setOpen(false)} type="button">×</button></header><form className="admin-form" onSubmit={create}><div className="admin-form-grid"><label><span>裁判员</span><select name="refereeId" required>{referees.map((referee) => <option key={referee.id} value={referee.id}>{referee.label}</option>)}</select></label><label><span>类型</span><select name="kind"><option value="AVAILABLE">可执裁</option><option value="UNAVAILABLE">不可执裁</option></select></label><label><span>比赛制式</span><select name="competitionType"><option value="BOTH">均可</option><option value="ELEVEN_A_SIDE">十一人制</option><option value="FUTSAL">五人制</option></select></label><label><span>开始</span><input name="startAt" required type="datetime-local" /></label><label><span>结束</span><input name="endAt" required type="datetime-local" /></label></div><label><span>说明</span><input maxLength={240} name="note" /></label><footer><button className="admin-button admin-button-secondary" onClick={() => setOpen(false)} type="button">取消</button><button className="admin-button" type="submit">保存记录</button></footer></form></div></div> : null}
  </>;
}
...
export function AffiliationsManager({
  ...
  return <>
    ...
    {tab === "teams" ? <section className="admin-panel"><header className="admin-panel-header"><div><h2>参赛球队与组织关联</h2><p>管理参赛球队与学院、书院之间的组织关系，用于裁判选派时判断组织关联。普通学院/书院代表队可在创建比赛时按需自动建立。</p></div><div className="admin-page-actions"><button className="admin-button admin-button-secondary" onClick={() => setModal("from-units")} type="button">从组织创建</button><button className="admin-button admin-button-secondary" onClick={() => setModal("joint")} type="button">+ 联合队</button><button className="admin-button" onClick={() => setModal("bulk")} type="button">批量导入球队</button></div></header>{!competitions.length ? <div className="admin-empty-state"><strong>请先创建赛事后再创建参赛球队</strong><p>赛事是球队与比赛的上级对象。</p><Link className="admin-button" href="/admin/competitions/new">前往赛事管理</Link></div> : <div className="admin-affiliation-list">{teams.map((team) => <form className="admin-affiliation-row admin-team-affiliation-row" key={team.id} onSubmit={(event) => { event.preventDefault(); const form = new FormData(event.currentTarget); void run(() => api("/api/referees/admin/team-affiliations", "PUT", { teamId: team.id, teamType: form.get("teamType"), unitIds: form.getAll("unitIds") }), "球队组织关联已保存。"); }}><div><strong>{team.name}</strong><span>{team.competition}</span></div><select defaultValue={team.teamType} name="teamType"><option value="ORGANIZATION">固定组织代表队</option><option value="JOINT">联合队</option><option value="FREEFORM">自由组队</option></select><select defaultValue={team.unitIds} multiple name="unitIds"><AdminAffiliationOptionGroups options={units} /></select><button className="admin-button admin-button-secondary" type="submit">保存</button></form>)}</div>}</section> : null}
...
export function ConflictReportsManager({
  reports,
}: {
