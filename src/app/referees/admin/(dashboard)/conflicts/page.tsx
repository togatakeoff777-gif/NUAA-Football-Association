...
export default async function AdminConflictReportsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <>
    ...
    <nav className="admin-tabs admin-route-tabs">{[["PENDING","待处理"],["HANDLED","已处理"],["ALL","全部"]].map(([value,label]) => <Link aria-current={status === value ? "page" : undefined} href={`/admin/conflicts?status=${value}`} key={value}>{label}</Link>)}</nav>
