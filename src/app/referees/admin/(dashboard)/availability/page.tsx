...
export default async function AdminAvailabilityPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <>
    ...
    <AdminPanel title={`时间记录 · ${records.length}`} description="时间按 Asia/Shanghai 展示，最多显示 300 条。"><AvailabilityManager records={records.map((item) => ({ id: item.id, refereeId: item.refereeId, referee: `${item.referee.publicCode} · ${item.referee.name}`, kind: item.kind, competitionFormat: item.competitionFormat, startAt: formatRefereeDateTime(item.startAt), endAt: formatRefereeDateTime(item.endAt), note: item.note ?? "" }))} referees={referees.map((item) => ({ id: item.id, label: `${item.publicCode} · ${item.name}` }))} /></AdminPanel>
  </>;
}
