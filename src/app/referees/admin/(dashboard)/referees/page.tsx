...
export default async function AdminRefereesPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const status = ["PENDING", "ACTIVE", "INACTIVE", "ARCHIVED"].includes(rawStatus) ? rawStatus : "";
  ...
  const [colleges, referees] = await Promise.all([
    ...
    prisma.referee.findMany({
      where: {
        ...
        ...(collegeId ? { collegeId } : {}),
        ...(status ? { status: status as "PENDING" | "ACTIVE" | "INACTIVE" | "ARCHIVED" } : {}),
        ...(format ? { capabilities: { some: { format: format as "ELEVEN_A_SIDE" | "FUTSAL" } } } : {}),
      },
      select: { id: true, name: true, publicCode: true, studentId: true, refereeLevel: true, status: true, college: { select: { name: true } }, capabilities: { select: { format: true } } },
  ...
  return <>
    ...
    <form className="admin-filter-bar">
      ...
      <label><span>资格</span><select defaultValue={format} name="format"><option value="">全部制式</option><option value="ELEVEN_A_SIDE">十一人制</option><option value="FUTSAL">五人制</option></select></label>
    ...
    <AdminPanel title={`裁判员名录 · ${referees.length}`} description="列表不展示手机、QQ 和内部备注。">
      {referees.length ? <div className="admin-table-scroll"><table className="admin-data-table"><thead><tr><th>姓名</th><th>裁判员编号</th><th>学院</th><th>裁判等级</th><th>十一人制</th><th>五人制</th><th>状态</th><th>操作</th></tr></thead><tbody>{referees.map((referee) => {
        const formats = new Set(referee.capabilities.map((item) => item.format));
        return <tr key={referee.id}><td><strong>{referee.name}</strong><small>{referee.studentId || "未登记学号"}</small></td><td>{referee.publicCode}</td><td>{referee.college?.name ?? "待确认"}</td><td>{referee.refereeLevel || "—"}</td><td>{formats.has("ELEVEN_A_SIDE") ? "具备" : "—"}</td><td>{formats.has("FUTSAL") ? "具备" : "—"}</td><td><AdminStatusBadge status={referee.status} label={refereeStatusLabels[referee.status]} /></td><td><div className="admin-table-actions"><Link href={`/referees/admin/referees/${referee.id}`}>查看</Link><Link href={`/referees/admin/referees/${referee.id}#profile`}>编辑</Link></div></td></tr>;
